...
This is my emacs configuration in the form of an org file. It is meant to be used with ob-tangle
to extract the actual configuration (to default.el). Since I also use NixOS it is together with
nix expressions this is meant to happen.
...
The famous org. Default settings I use and stuff.
...
In a terminal, TAB corresponds to C-i so that's an issue. Otherwise, for the GUI emacs,
which I normally use, this isn't a problem.
...
So evil is vim for emacs. A better vim basically ;-).
...
Evil nerd commenter let's me comment out one of more lines. In Evil mode this is done using C-c i (multiple lines by
selecting a region).
...
Evil keybindings for org-mode. Yay.
...
Allows integrating nix with emacs - project specific.
...
Show documentation popups nixos configuration options.
...
Modes for additional languages.
...
This enables syntax checking / linting for moonscript. Defined right here. Disabled
until I get around to adding the dependencies.
...
For showing errors in terminal (pos-tip - see below - doesn't do that).
...
For showing errors under point. Refers to above for terminal above.
...
Electric pair-mode will help with matching parentheses, quotes etc. Only in prog mode though thanks.
...
To make themes work nicely with emacsclient we need to set the theme when the client connects. It's
...
What's the browser you use? I use a shell script called browse which deals with all that.
...
#+BEGIN_SRC nix :tangle pkgs.nix
{ config, pkgs, fetchFromGitHub, emacsConfig, ...}:
rec {
